    ...
    for file in files:
        ...
        if os.path.isfile(file_path):
            for key, value in cust_dirs.items():
                if os.path.splitext(file)[1].lstrip('.') in list(value) or os.path.splitext(file)[1].lstrip(
                        '.').upper() in list(value):
...
def unpacking(path):
    ...
    if os.path.exists(arch_path) and os.listdir(arch_path):
        for file in os.listdir(arch_path):
            if os.path.splitext(file)[1].lstrip('.') in cust_dirs['архивы'] or os.path.splitext(file)[1].lstrip(
                    '.').upper() in cust_dirs['архивы']:
